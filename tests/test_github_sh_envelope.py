...
def assert_curl_auth_uses_stdin_config() -> None:
    ...
    assert "secret-token" not in curl_cmd, "GitHub token leaked through curl argv"
    assert "--config" in curl_cmd and "-" in curl_cmd, "curl must read auth config from stdin"
...
def assert_optional_fetch_status_semantics() -> None:
    ...
    with mock.patch.object(_envelope, "fetch_json_with_status", side_effect=lambda endpoint: next(responses)):
