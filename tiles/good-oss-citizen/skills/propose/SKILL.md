...
- **Front matter and UI labels.** For Markdown issue templates with YAML front matter such as `name:` or `labels:`, keep the body headings from the Markdown content, note any labels the contributor should apply through the GitHub UI, and explicitly tell the contributor which labels from front matter to apply through the GitHub UI. Do not silently drop label/front-matter intent from the handoff, and do not assume front matter will be preserved after the issue body is pasted into GitHub.

State which template was selected in your handoff and why (e.g., "Used `<chosen-filename>` because it matches the intent of this contribution: <reason>"). If multiple templates were available, name the candidate paths you considered and name every candidate template path returned by the helper before the selected path, including non-selected templates such as bug reports when drafting a feature request. Reference each template by its actual path; do not invent a filename.

For Markdown issue templates, also add a short HTML metadata comment at the top of `issue_body.md` before the template headings. This is not a template section and should not replace any required content. Include:
- `Template selected: <path> because <reason>`
- `Candidate templates considered: <path>, <path>, ...`
- `GitHub UI labels to apply: <labels from front matter>` when front matter declares labels

For YAML issue forms, add the same short HTML metadata comment before the field-by-field answers. Include `Template selected: <path> fetched with templates-issue` so the artifact records the actual form source, then map the body to the declared YAML field ids/labels. Do not replace the YAML field answers with generic Markdown headings.
...
- Do not leave empty bullets, blank headings, HTML comments, or placeholder labels as section content. For Environment/version sections, first use concrete values available from the task, local context, or repository metadata: for example, the Python version from the local runtime if relevant, the package version from `pyproject.toml` / package metadata, or a repository commit/ref if no release version is visible. Only use `Unknown from report` when no concrete value can be found after checking available context, and include the exact detail the contributor should replace before submitting. Avoid `Not provided yet` wording, which reads like a placeholder instead of a filled answer.
- **Include an AI disclosure section in the issue body — same as for PR descriptions.** This applies even when the issue template doesn't have a dedicated disclosure slot. For markdown templates: add the disclosure at the bottom of the issue body. For YAML form templates: paste the disclosure into the most relevant freeform field (e.g., "additional context" or the last textarea) or append it after the form output. **If `AI_POLICY.md` defines a disclosure format** (Tool / Used for / Human-verified fields, or any structured template), you MUST use that exact structure — copy the template from `disclosure-format.data.template` verbatim and fill in the fields. If `disclosure-format.data.format` is `code_block`, the issue body must include the filled disclosure as a fenced code block in that declared format, not as a generic Markdown heading or prose paragraph. For voluntary disclosure or any non-`code_block` project format, write normal Markdown/prose, not a fenced code block. Never wrap voluntary disclosure in triple backticks; a fenced voluntary disclosure is wrong because it renders as code instead of a readable disclosure section. If the disclosure has fields like `Tool`, `Used for`, and `Human verified`, write them as normal Markdown bullets or lines under `## AI Disclosure`, not inside ``` fences. This no-fence rule applies to the actual contents of `issue_body.md` even if the surrounding chat response previews that file in a code block. Do NOT substitute a generic prose sentence ("drafted with Claude, reviewed by the contributor") when a structured format is required; the judge compares against the project's declared format. Only if no AI policy exists, use voluntary prose disclosure.
- Before finishing, inspect `issue_body.md`. Unless the target repo explicitly required `disclosure-format.data.format == code_block`, the AI disclosure section must not appear between any ``` fence lines. If it is fenced, rewrite that section as normal Markdown and re-check the file.
